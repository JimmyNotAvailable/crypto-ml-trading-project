...
# Add correct path to project root
...
if all(key in datasets for key in ['X_train', 'y_train']):
    ...
    print(f"  y_train_price range: [{y_train_price.min():.4f}, {y_train_price.max():.4f}]")
...
if all(key in datasets for key in ['test_df', 'feature_cols']):
    ...
    if all(key in datasets for key in ['X_test', 'y_test']):
        ...
        elif hasattr(y_test_direct, 'columns') and 'price' in y_test_direct.columns:
            y_test_direct = y_test_direct['price']

        print(f"Direct from datasets X_test: {X_test_direct.shape}")
        print(f"Direct from datasets y_test: {y_test_direct.shape}")
        print(f"From test_df X_test: {X_test.shape}")
        print(f"From test_df y_test: {y_test_price.shape}")

        print(f"\nAre X_test equal? {np.allclose(X_test_direct, X_test.values, equal_nan=True)}")
        print(f"Are y_test equal? {np.allclose(y_test_direct, y_test_price, equal_nan=True)}")
        ...
        print(f"y_test_direct range: [{y_test_direct.min():.4f}, {y_test_direct.max():.4f}]")
